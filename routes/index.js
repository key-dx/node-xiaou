...
router.get("/getseckill",async(req,res)=>{
    ...
    var data = [];
    var sql = `SELECT * FROM ${tableNameSeck} WHERE begintime <= ${start} AND endtime >= ${start} LIMIT 1`;
    data = await Db.select(req,sql);
    if(data){
        sql= `SELECT id,price,market_price,goodsname,img FROM ${tableNameGoods} WHERE id IN(${data[0].goodsid})`;
        var goods = await Db.select(req,sql);
        data[0].goodsarr = goods;
    }
    res.send(Success(data));
})
//获取首页商品-推荐、最新上架、所有商品
...
router.get("/getcategoods",async(req,res)=>{
    ...
    if(fid || sid){
		let data = [];
		if(fid){
			data = await Db.select(req, `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND first_cateid = ${fid}`);
			res.send(Success(data));
			return;
		}
		if(sid){
			data = await Db.select(req, `SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND second_cateid = ${sid}`);
			res.send(Success(data));
			return;
		}
