...
router.get("/userlist", async (req, res) => {
    ...
    let sql = '';
    if(size && page){
        sql = `SELECT a.*,b.rolename FROM ${tableName} a LEFT JOIN ${tableNameRole} b ON a.roleid = b.id ORDER BY a.id LIMIT ${(page-1)*size},${size}`;
    }else{
        sql = `SELECT a.*,b.rolename FROM ${tableName} a LEFT JOIN ${tableNameRole} b ON a.roleid = b.id ORDER BY a.id`;
    }
...
router.post("/userdelete", async (req, res) => {
    ...
    if (!uid) {
        ...
        if(result === true){
            data = await Db.select(req, `SELECT a.*,b.rolename FROM ${tableName} a LEFT JOIN ${tableNameRole} b ON a.roleid = b.id ORDER BY a.id`);
            res.send(Success(data))
        }else{
            res.send(MError());
        }
